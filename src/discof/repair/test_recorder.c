...
test_recorder_peer_basic( fd_wksp_t * wksp ) {
  ...
  /* adding peers */
  fd_recorder_peer_t * peer1 = fd_recorder_peer_add( ledger, &peer1_key, peer1_ip, current_time );
  FD_TEST( peer1 );
  ...
  fd_recorder_peer_t * peer2 = fd_recorder_peer_add( ledger, &peer2_key, peer2_ip, current_time );
  FD_TEST( peer2 );
  ...
  fd_recorder_peer_t * not_found = fd_recorder_peer_query( ledger, &nonexistent_key );
  FD_TEST( !not_found );
  ...
  fd_recorder_peer_t * peer1_updated = fd_recorder_peer_add( ledger, &peer1_key, peer1_new_ip, new_time );
  FD_TEST( peer1_updated );
  FD_TEST( peer1_updated == peer1 ); /* Should be same object */
  FD_TEST( fd_recorder_peer_cnt( ledger ) == 2UL ); /* Count shouldn't change */
  ...
  /* removing peers */
  fd_recorder_peer_t * remove_result = fd_recorder_peer_remove( ledger, &peer1_key, 1);
  ...
  fd_recorder_peer_t * remove_result2 = fd_recorder_peer_remove( ledger, &nonexistent_key, 1);
...
test_recorder_req_basic( fd_wksp_t * wksp ) {
  ...
  long current_time = 1000000000L;
  fd_recorder_peer_t * peer = fd_recorder_peer_add( ledger, &peer_key, peer_ip, current_time );
  FD_TEST( peer );
  ...
  fd_recorder_req_t * req1 = fd_recorder_req_insert( ledger, nonce1, timestamp, 
  ...
  fd_recorder_req_t * req2 = fd_recorder_req_insert( ledger, nonce2, timestamp + 1000000UL,
  ...
  fd_recorder_req_t * duplicate = fd_recorder_req_insert( ledger, nonce1, timestamp,
                                                                   &peer_key, peer_ip, slot, shred_idx, req_type );
  FD_TEST( !duplicate );
  FD_TEST( fd_recorder_req_cnt( ledger ) == 2UL ); /* Count shouldn't change */
...
test_recorder_peer_selection( fd_wksp_t * wksp ) {
  ...
  for( uint i = 0; i < 5; i++ ) {
    ...
    fd_recorder_peer_t * peer = fd_recorder_peer_add( ledger, &peer_keys[i], peer_ip, current_time );
    FD_TEST( peer );
  }
  ...
  fd_recorder_select_peers( ledger, 3, selected_peers );
  ...
  fd_recorder_select_peers( ledger, 3, more_selected );
...
test_recorder_peer_update( fd_wksp_t * wksp ) {
  ...
  fd_recorder_peer_t * updated_peer = fd_recorder_peer_update( ledger, &peer_key, peer_ip, 1, (ulong)update_time, (ulong)current_time );
  ...
  fd_recorder_peer_t * sent_peer = fd_recorder_peer_update( ledger, &peer_key, peer_ip, 0, (ulong)send_time, (ulong)current_time );
  FD_TEST( sent_peer );
